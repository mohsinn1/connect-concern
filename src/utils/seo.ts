export function generateCanonicalUrl(siteUrl: string, pathname: string, siteEnv: string, seoApproved: boolean): string {
  let baseUrl = siteUrl || 'https://nexaconcerns.site';
  ...
  try {
    ...
    url = new URL('https://nexaconcerns.site');
  }
  ...
  if (siteEnv === 'production' && seoApproved) {
    if (url.hostname.includes('localhost') || url.hostname.includes('nexaconcerns.site')) {
      return 'https://nexaconcerns.site' + cleanPath;
    }
  }
