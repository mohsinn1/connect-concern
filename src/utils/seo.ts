export function generateCanonicalUrl(siteUrl: string, pathname: string, siteEnv: string, seoApproved: boolean): string {
  let baseUrl = siteUrl || 'https://nexaconcern.site';
  ...
  try {
    ...
    url = new URL('https://nexaconcern.site');
  }
  ...
  if (siteEnv === 'production' && seoApproved) {
    if (url.hostname.includes('localhost') || url.hostname.includes('nexaconcern.site')) {
      return 'https://nexaconcern.site' + cleanPath;
    }
  }
