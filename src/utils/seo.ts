export function generateCanonicalUrl(siteUrl: string, pathname: string, siteEnv: string, seoApproved: boolean): string {
  let baseUrl = siteUrl || 'https://novaconcerns.site';
  ...
  try {
    ...
    url = new URL('https://novaconcerns.site');
  }
  ...
  if (siteEnv === 'production' && seoApproved) {
    if (url.hostname.includes('localhost') || url.hostname.includes('novaconcerns.site')) {
      return 'https://novaconcerns.site' + cleanPath;
    }
  }
